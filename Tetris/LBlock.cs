...
namespace Tetris
{
    public class LBlock : Tetrimino
    {
        public LBlock(System.Windows.Controls.Canvas gameCanvas, GameArray gameArray)
        {
            this.Type = "L";
            this.CanMove = true;
        ...
        public override int[] GenerateYArray(int y, int orientation)
        {
            if (orientation == 0)
            {
                return new int[] { y - 20, y, y + 20, y + 20 };
            }
            else if (orientation == 1)
            {
                return new int[] { y, y, y, y + 20 };
            }
            else if (orientation == 2)
            {
                return new int[] { y - 20, y - 20, y, y + 20 };
            }
            else if (orientation == 3)
            {
                return new int[] { y - 20, y, y, y };
            }
            else
            {
                return null;
        ...
        public override void DrawBlock(int x, int y)
        {
            ...
            fillBrush.Color = Color.FromArgb(255, 220, 20, 0);
        ...
        public override void PlaceBlock(int x, int y)
        {
            ...
            SolidColorBrush fillBrush = new SolidColorBrush();
            SolidColorBrush strokeBrush = new SolidColorBrush();
            fillBrush.Color = Color.FromArgb(255, 80, 0, 0);
            strokeBrush.Color = Color.FromArgb(255, 1, 1, 0);
            rect.Height = 20;
            rect.Width = 20;
            rect.StrokeThickness = 1;
